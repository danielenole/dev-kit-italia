...
## Accordion item
E' il singolo elemnto collassabile all'interno dell'accordion .
...
Nel caso in cui venga usato l'attributo `left-icon`, le icone sulla sinistra saranno `+` per espandere l'elemento e `-` per collassarlo. 
Nel default invece (quando le icone sono sulla destra), verranno mostrate le icone di `freccia giu` per espandere l'elemento e `freccia su` per collassarlo.
