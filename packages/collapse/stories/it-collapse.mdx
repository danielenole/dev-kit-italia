...
Su questo componente si basa anche il [componente Accordion](?path=/docs/componenti-accordion--documentazione), usando i collapse in gruppo in gruppo permettendo di attivarli indipendentemente l’uno dall’altro oppure in modo esclusivo con l’attivazione di solo un blocco alla volta.
